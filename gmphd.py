...
[(float(comp.loc), comp.weight) for comp in g.gmm]
	...
	def __init__(self, birthgmm, survival, detection, f, fx, q, h, hx, r, clutter):
		"""
		...
		self.fx = fx # system function without process noise
		self.f = f  # Jacobian of fx
		self.q = array(q, dtype=myfloat)   # process noise covariance     (Q_k-1 in paper)
		self.hx = hx # observation function without measurement noise
		self.h = h # Jacobian of hx
	...
	def update(self, dt, obs):
		...
		for comp in self.gmm:
			f_ekf, g_ekf = self.f(comp.loc, dt)
			newP = dot(dot(g_ekf , self.q) , g_ekf.T) + dot(dot(f_ekf, comp.cov), f_ekf.T)
			newP = (newP + newP.T)/2

			newgmm = GmphdComponent(self.survival * comp.weight, self.fx(comp.loc, dt), newP)                

			updated.append(newgmm)
		...
		for comp in predicted:
			eta_k = self.hx(comp.loc)

			H_ekf, U_ekf = self.h(comp.loc)
			S_k = dot(dot(U_ekf, self.r), U_ekf.T) + dot(dot(H_ekf, comp.cov), H_ekf.T)
			S_k = (S_k + S_k.T)/2 # to avoid numerical problem

			K_k = dot(dot(comp.cov , H_ekf.T ), linalg.inv(S_k))

			P_k = dot(eye(comp.cov.shape[0]) - dot(K_k, H_ekf), comp.cov)
...
# to-do
	def prune(self, truncthresh=1e-6, mergethresh=0.01, maxcomponents=100):
		...
		sourcegmm = [deepcopy(comp) for comp in self.gmm if comp.weight > truncthresh]
